...
def search_step(pipeline_list, heuristics, list_of_moves):
    ...
    if board[4] == 0:
        add_to_pipeline(pipeline_list, swap(board_wrapper, 4, 1), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 4, 3), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 4, 5), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 4, 7), heuristics, list_of_moves)
    elif board[0] == 0:
        add_to_pipeline(pipeline_list, swap(board_wrapper, 0, 1), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 0, 3), heuristics, list_of_moves)
    elif board[2] == 0:
        add_to_pipeline(pipeline_list, swap(board_wrapper, 2, 1), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 2, 5), heuristics, list_of_moves)
    elif board[6] == 0:
        add_to_pipeline(pipeline_list, swap(board_wrapper, 6, 3), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 6, 7), heuristics, list_of_moves)
    elif board[8] == 0:
        add_to_pipeline(pipeline_list, swap(board_wrapper, 8, 5), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 8, 7), heuristics, list_of_moves)
    elif board[1] == 0:
        add_to_pipeline(pipeline_list, swap(board_wrapper, 1, 4), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 1, 0), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 1, 2), heuristics, list_of_moves)
    elif board[3] == 0:
        add_to_pipeline(pipeline_list, swap(board_wrapper, 3, 4), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 3, 0), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 3, 6), heuristics, list_of_moves)
    elif board[5] == 0:
        add_to_pipeline(pipeline_list, swap(board_wrapper, 5, 4), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 5, 8), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 5, 2), heuristics, list_of_moves)
    elif board[7] == 0:
        add_to_pipeline(pipeline_list, swap(board_wrapper, 7, 4), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 7, 6), heuristics, list_of_moves)
        add_to_pipeline(pipeline_list, swap(board_wrapper, 7, 8), heuristics, list_of_moves)
...
def slide(start_board_wrapper, current_heuristic, pipelineList):  # finally the searching method: start is the the board to be checked with certain
    # heuristic
    counter = 0
    list_of_moves = []
    add_to_pipeline(pipelineList, start_board_wrapper, current_heuristic, list_of_moves)  # add the first board to the pipeline
    current_board_wrapper = search_step(pipelineList, current_heuristic, list_of_moves)  # introduce neighbours into pipeline
    for counter in range(182000):
        ...
        current_board_wrapper = search_step(pipelineList, current_heuristic, list_of_moves)  # repeat introducing boards into pipeline
    list_of_moves.clear()
...
def run_puzzle_solver(board_wrapper_list, current_heuristic, result, index):
    ...
    print(result[index][0][0].previous_board.board)
...
if __name__ == '__main__':
    ...
    astar = multiprocessing.Process(target=run_puzzle_solver, args=(boards100, heuristic.astar, results, 0))
    manhattan = multiprocessing.Process(target=run_puzzle_solver, args=(boards100, heuristic.manhattan, results, 1))
    hamming = multiprocessing.Process(target=run_puzzle_solver, args=(boards100, heuristic.hamming, results, 2))
